...
public final class MandrillSendersApiTest extends MandrillTestCase {
	...
	public void testAddDomain() throws IOException, MandrillApiError {
	...
	@Test(expected=MandrillApiError.class)
	public void testTimeSeries01() throws MandrillApiError, IOException {
		mandrillApi.senders().timeSeries(null);
		Assert.fail();
	}

	@Test
	public void testTimeSeries02() throws MandrillApiError, IOException {
		final MandrillSender[] senders = mandrillApi.senders().list();
		Assert.assertNotNull( senders );
		if(senders.length > 0) {
